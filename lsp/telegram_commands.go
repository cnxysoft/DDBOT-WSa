    ...
    lsptelegram.StartReceiving(func(chatID int64, fromID int64, text string) {
            ...
            for i := 0; i < len(args); i++ {
                ...
                default:
                    if !strings.HasPrefix(a, "-") && target == 0 {
                        if v, err := strconv.ParseInt(a, 10, 64); err == nil { target = v }
                    }
                }
            }
        ...
        case "config":
            ...
            for idx < len(args) && strings.HasPrefix(args[idx], "-") { // skip -g/-s/-t tokens
                if (args[idx] == "-g" || args[idx] == "-s" || args[idx] == "-t") && idx+1 < len(args) { idx += 2 } else { idx++ }
            }
            if idx >= len(args) { c.TextReply("参数错误"); return }
            ...
            case "at":
                ...
                if action == "add" || action == "remove" {
                    for _, s := range rest[2:] { if v, err := strconv.ParseInt(s, 10, 64); err == nil { qqs = append(qqs, v) } }
                }
            ...
            case "filter":
                ...
                switch fsub {
                case "type":
                    if len(rest) < 2 { c.TextReply("用法: /config -g <群号> filter type <id> [types...]"); return }
                    id := rest[1]
                    types := rest[2:]
                    IConfigFilterCmdType(c, group, id, normSite, ctype, types)
                case "not_type":
                    if len(rest) < 2 { c.TextReply("用法: /config -g <群号> filter not_type <id> [types...]"); return }
                    id := rest[1]
                    types := rest[2:]
                    IConfigFilterCmdNotType(c, group, id, normSite, ctype, types)
                case "text":
                    if len(rest) < 2 { c.TextReply("用法: /config -g <群号> filter text <id> [keywords...]"); return }
                    id := rest[1]
                    keywords := rest[2:]
                    IConfigFilterCmdText(c, group, id, normSite, ctype, keywords)
                case "clear":
                    if len(rest) < 2 { c.TextReply("用法: /config -g <群号> filter clear <id>"); return }
                    id := rest[1]
                    IConfigFilterCmdClear(c, group, id, normSite, ctype)
                case "show":
                    if len(rest) < 2 { c.TextReply("用法: /config -g <群号> filter show <id>"); return }
                    id := rest[1]
        ...
        case "noupdate", "no_update":
            ...
            if del {
                ...
                if err == nil { c.TextReply("成功 - 您将接收到更新消息") }
            } else {
                err = localdb.Set(key, "")
                if err == nil { c.TextReply("成功 - 您将不再接受更新消息") }
            }
...
func (l *Lsp) newTGContext(chatID, fromID, senderUin, groupCode int64) *MessageContext {
    ...
    c.DisabledReply = func() interface{} { lsptelegram.SendToChat(chatID, mmsg.NewText("失败 - 该命令已禁用")); return nil }
    c.GlobalDisabledReply = func() interface{} { lsptelegram.SendToChat(chatID, mmsg.NewText("失败 - 管理员已禁用该命令")); return nil }
    return c
}
